...
import { PangoChefUserStorage } from "./PangoChefUserStorage.sol";
import "./PangoChefStructs.sol";
...
contract PangoChef is PangoChefFunding, ReentrancyGuard {
    ...
    /**
     * @notice Public view function to get the info about a user of a pool
    ...
    function userRewardRate(uint256 poolId, address userId) external view returns (uint256) {
        ...
        uint256 poolValue = _getValue(pools[poolId].valueVariables);
        uint256 userValue = _getValue(_getUser(poolId, userId).valueVariables);
    ...
    function userPendingRewards(uint256 poolId, address userId) external view returns (uint256) {
        ...
        Pool storage pool = pools[poolId];
        UserPool memory user = _getUser(poolId, userId);
    ...
    function poolZeroLockCount(address user) public view returns (uint256) {
        address userStorageContract = _getUserStorageContract(user);
        return userStorageContract.code.length == 0
            ? 0
            : PangoChefUserStorage(userStorageContract).poolZeroLockCount();
    }
    ...
    function createUserStorageContract() external {
        //address userStorageContract = _getUserStorageContract(msg.sender);
        //if (userStorageContract.code.length != 0) revert NoEffect();
        Create2.deploy(
            0,
            bytes32(bytes20(msg.sender)),
            type(PangoChefUserStorage).creationCode
        );
    }
    ...
    function _withdraw(uint256 poolId, uint256 amount) private {
        ...
        Pool storage pool = pools[poolId];
        UserPool memory user = _getUser(poolId, msg.sender);
        ...
        // Snapshot the lastUpdate and summations.
        _snapshotRewardSummations(pool, user);
        _setUser(poolId, msg.sender, user);
     ...
     */
    function _incrementLockOnPoolZero(UserPool memory user) private {
        ...
        if (!user.isLockingPoolZero) {
            ...
            address userStorageContract = _getUserStorageContract(msg.sender);
            PangoChefUserStorage(userStorageContract).incrementLock();
     ...
     *             properties of the pool must belong to the caller.
     */
    function _decrementLockOnPoolZero(uint256 poolId, UserPool memory user) private {
        address userStorageContract = _getUserStorageContract(msg.sender);
        if (poolId == 0) {
            // Ensure pool zero is not locked.
            if (PangoChefUserStorage(userStorageContract).poolZeroLockCount() != 0) revert Locked();
        } else if (user.isLockingPoolZero) {
            // Decrement lock count on pool zero if this pool was locking it.
            PangoChefUserStorage(userStorageContract).decrementLock();
     ...
     */
    function _snapshotRewardSummations(Pool storage pool, UserPool memory user) private view {
    ...
    function _getDeltaRewardSummations(
        ...
        RewardSummations memory rewardSummationsPaid = user.rewardSummationsPaid;
        RewardSummations storage rewardSummationsStored = pool.rewardSummationsStored;
     ...
     * @return The accrued rewards of the position.
     */
    function _earned(RewardSummations memory deltaRewardSummations, UserPool memory user)
        private
        pure
        returns (uint256)
    {
